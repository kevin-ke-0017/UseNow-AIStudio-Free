...
本产品是**付费授权软件**，按设备授权：首次打开会显示本机**机器码**，
用购买时拿到的**兑换码**一键自动激活，或者把机器码发给卖家换取**激活码**手动激活。
**激活后授权不再联网验证**，不会再问第二次。
注意这说的是**授权** —— 对话 / 画图 / 视频都要调用 AI 接口，**用 AI 功能仍然需要联网**；
不联网时软件能打开，本地知识库、历史记录、导出都能用。

机器码只是设备标识的哈希，**不含任何硬件信息，也不含你的 API Key 和提示词**，可以放心发出。

**一个兑换码：同时 1 台电脑 + 1 台手机。** 电脑和手机各自独立计数：
...
所以**一个码就能同时用在你的电脑和手机上**，这不占用任何换绑次数 —— 两边分开算。
电脑那 2 次换绑用完之后，手机那 2 次依然是满的，反之同理。

重装软件、清数据、换网卡都**不算换绑**（同一台设备再来会把原来那张原样重发）；
只有重装系统 / 手机恢复出厂 / 真的换了设备才会消耗一次。换绑次数用完了就联系卖家。

> **老用户升级请按顺序来**：先在旧版【设置】→【💾 数据备份与恢复】点「📤 导出全部数据」，
> 再卸载旧版、安装新版、激活，最后「📥 导入并恢复」。本版不会自动继承旧版授权。
...
任选一家：Agnes AI、DeepSeek、OpenAI、Gemini、Claude、通义千问、智谱 GLM……软件内的「⚙️ 设置 → 快捷添加」已经预置好了这些服务商的地址和常用模型，选一个、粘贴 Key 即可。
...
| 用途 | 是否需要付费 |
| --- | --- |
| **个人使用**（含用它接活赚钱） | 🔑 **需购买激活码**（一次买断，永久有效）—— 用它接活、做自媒体、给客户干活赚的钱**不额外分成** |
| **企业 / 团队内部使用** | 需要商用授权 |
| **定制开发 / 换品牌 / 私有化部署** | 定制服务 |
| **转卖、二次分发、逆向** | ❌ 不允许 |
